...
local function attachControls(parent, markNoDrag)
	markNoDrag = markNoDrag or function() end
	...
	function controls:AddFolder(folder_name)
		...
		return attachControls(objects, markNoDrag), wrap
	end
...
function lib:AddWindow(title, options)
	...
	local function isScrollAreaAt(pos)
		for _, desc in ipairs(Window:GetDescendants()) do
			if desc:IsA("ScrollingFrame") and desc.Name ~= "TabButtons" and desc.ScrollingEnabled ~= false and isVisibleInWindow(desc) and pointInGui(desc, pos) then
				return true
			end
		end
		return false
	...
	function window_data:AddTab(tab_name)
		...
		local tab_data = attachControls(new_tab, markNoDrag)
